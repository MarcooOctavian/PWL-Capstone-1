...
<head>
    ...
    <style>
        ...
        .qr-container { text-align: center; margin: 30px 0; padding: 20px; border: 2px dashed #007bff; border-radius: 8px; background-color: #f9fbff; }
...
<div class="email-wrapper">
    <div class="header">
        ...
        <p style="margin-top: 5px; opacity: 0.9;">{{ $ticket->transaction->typeTicket->event->title }}</p>
    </div>

    <div class="content">
        <p>Halo, <strong>{{ $ticket->transaction->user->name }}</strong>,</p>
        ...
        <div class="qr-container">
            ...
            <h3 style="margin-bottom: 0; letter-spacing: 2px;">{{ $ticket->ticket_code }}</h3>
        </div>
        ...
        <div class="ticket-details">
            <table>
                <tr>
                    <th>Jenis Tiket</th>
                    <td>{{ $ticket->transaction->typeTicket->name }}</td>
                </tr>
                <tr>
                    <th>Tanggal Event</th>
                    <td>{{ \Carbon\Carbon::parse($ticket->transaction->typeTicket->event->date)->translatedFormat('l, d F Y - H:i') }}</td>
                </tr>
                <tr>
                    <th>Lokasi</th>
                    <td>{{ $ticket->transaction->typeTicket->event->location->venue_name ?? 'TBA' }}</td>
                </tr>
                <tr>
                    <th>No. Invoice</th>
                    <td>{{ $ticket->transaction->invoice_number }}</td>
                </tr>
            </table>
        </div>
